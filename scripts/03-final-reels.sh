#!/bin/bash
# Stage 3a: graded master -> Reels/Stories delivery (9:16, 1080x1920).
# Usage: ./03-final-reels.sh IMG_XXXX
set -euo pipefail
...
require_portrait "$IN"   # refuses landscape rather than silently squashing it — see lib.sh

# Grain and sharpen are applied AFTER the downscale, not before — grain sized for the 4K master
# gets crushed/invisible once scaled to 1080p, and sharpening pre-resize just gets blurred back
# out by the resize (see docs/PIPELINE.md).
#
# zscale (not scale) does the downscale because only zscale actually DITHERS the 10->8 bit
# reduction. Verified: `scale=...,format=yuv420p` and `-sws_dither ed` produce byte-identical
# output (no dithering at all), while zscale's error_diffusion differs — and matters on this
# footage, which has a large flat sky where banding would show.
...
ffmpeg -y -i "$IN" -f lavfi -i "color=c=gray:s=540x960:r=${FPS}" \
	-filter_complex "[0:v]${STAB}${CHROMA}zscale=w=1080:h=1920:f=lanczos:d=error_diffusion,format=yuv420p,unsharp=5:5:0.4:5:5:0.0[b];\
[1:v]noise=c0s=${GRAIN_STRENGTH}:c0f=t,scale=1080:1920:flags=bilinear,format=yuv420p,setparams=colorspace=bt709:color_primaries=bt709:color_trc=bt709:range=limited[g];\
...
require_nonempty "$OUT" "reels/stories encode"
